...
def test_balanco_que_fecha_passa_nas_duas_identidades():
    ...
    assert ident.loc[0, "residuo_principal"] == pytest.approx(0.0)
    assert ident.loc[0, "residuo_decomposto"] == pytest.approx(0.0)
...
def test_resumo_conta_os_tres_estados():
    ident = _validar(fx.cenario_balanco_completo())
    assert validations.resumo(ident) == {"OK": 1, "FALHA": 0, "FALTANDO": 0}
